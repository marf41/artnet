...
func (an ArtNet) Channels(from, to int, delim string) string {
  n := to - from
  ch := make([]string, n)
  for i, v := range(an.Data[from:to]) { ch[i] = strconv.Itoa(int(v)) }
...
func Parse(buf []byte, n int, addr net.Addr, plog bool) (ArtNet, error) {
  s := ArtNet{}
  s.Source = addr
  if n < 14 { return s, notanerr(fmt.Sprintf("too short, %d bytes", n)) }
  isan := string(buf[:8]) == "Art-Net\x00"
  if !isan { return s, notanerr(fmt.Sprintf("header is %q", buf[:8])) }
  an := make([]uint8, n - 8)
  for i, b := range buf[8:n] { an[i] = uint8(b) }
  op := uint16(an[0]) * 256 + uint16(an[1])
  ver := uint16(an[2]) * 256 + uint16(an[3])
  s.OpCode = op
  s.ProtocolVer = ver
  if ver < 14 { return s, notanerr(fmt.Sprintf("version is %d", ver)) }
  if op == 0x20 {
    flags := an[4]
    pr := an[5]
    s.Pool = ArtNetPoll{}
    s.Pool.Priority = pr
    if plog { log.Println("ArtPoll packet.") }
    if flags > 0 {
  ...
  if op == 0x50 {
    ...
    if s.Length > 512 { return s, errors.New(fmt.Sprintf("Invalid packet (data length is %d).\n", s.Length)) }
    for i, v := range an[10:10+s.Length] { s.Data[i] = v }
    if plog {
      // ch := make([]string, 16)
